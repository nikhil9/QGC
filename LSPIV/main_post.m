...
title_figure = 'Post-Processing:';

cell_list{1,1} = {'Quality Check and Averaging','quality_gui;'};
cell_list{2,1} = {'Velocity derived quantities','display_gui;'};
cell_list{3,1} = {'Model to prototype: Froude scaling','Froudesc_gui;'};
cell_list{4,1} = {'Discharge Computation','Q_gui;'};
cell_list{5,1} = {'Go back to previous menu','main_LSPIV;'};
cell_list{6,1} = {'Stop the application',['disp(''Bye. To run again, type Mat_LSPIV''); close(' num2str(fig_number) ');']}; %{'Exit','calib_gui;'};


show_window(cell_list,fig_number,title_figure,230,18,0,'clean',12);
