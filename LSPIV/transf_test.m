...
disp('  ')
display('#########################################');
display('Input the geometric transformation parameters:');
disp('  ')
disp('For getting help with the geometric transformation parameters, press h')
help_choice=input('Else, press any key?:   ','s');
disp('  ')
if (help_choice == 'h')
    figure(2);
    imshow('transf_help.bmp');
end

x_min__read=input('Xmin:  ')  ;
y_min__read=input('Ymin:  ') ;
x_max__read=input('Xmax:  ') ;
y_max__read=input('Ymax:  ') ;
resol=input('Resolution:  ') ;
min_coord=[];
min_coord(1)=x_min__read;
min_coord(2)=y_min__read;
max_ccord=[];
max_coord(1)=x_max__read;
max_coord(2)=y_max__read;
...
status = fclose('all');
disp('  ')
display('Check if the transformation parameters are OK')
disp('If not, do again step 2 of the LSPIV procedure')
disp('  ')
